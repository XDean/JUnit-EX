...
/**
 * Using <code>Suite</code> as a runner allows you to manually build a suite containing tests from many classes. It is
 * the JUnit 4 equivalent of the JUnit 3.8.x static {@link junit.framework.Test} <code>suite()</code> method. To use it,
 * annotate a class with <code>@RunWith(Suite.class)</code> and <code>@SuiteClasses({TestClass1.class, ...})</code>.
...
public class FriendlySuite extends FriendlyParentRunner<Runner> {
  ...
  /**
   * The <code>SuiteClasses</code> annotation specifies the classes to be run when a class annotated with
   * <code>@RunWith(Suite.class)</code> is run.
   */
  ...
  protected static Class<?>[] getAnnotatedClasses(Class<?> klass) throws InitializationError {
